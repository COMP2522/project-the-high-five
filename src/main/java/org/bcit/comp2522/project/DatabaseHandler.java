...
public class DatabaseHandler {
  ...
  /**
   * Constructs a new instance of DatabaseHandler
   * and establishes a connection to the MongoDB database.
   * The connection string used to connect includes
   * the username and password of the database user and the name
   * of the database to connect to.
   */
  public DatabaseHandler() {
    ConnectionString connectionString
            = new ConnectionString("mongodb+srv://" + username + ":" + password
  ...
  /**
   * Retrieves the top 8 high scores from the
   * "highScores" collection in the database, sorted in descending order by
   ...
   * If the
   *
   * @param jsonObject a JSONObject containing the current game state data.
   */
   ...
   * and creates a new GameGate object with the retrieved data.
   *
   * @param window the current game window
   * @return a GameState object with the current game
   * state data, or null if the game state not found in the database.
   */
